...
// ── Desktop Context Menu (three-dot ⋮) — Favourite only ──────────────────────
function SongContextMenu({ song, userId, onClose }) {
  const ref = useRef(null)
  const [favState, setFavState] = useState(null) // null = loading, true/false = state
  const [favLoading, setFavLoading] = useState(false)
  const [toast, setToast] = useState("")

  // Check current favourite status
  useEffect(() => {
    if (!userId || !getToken()) { setFavState(false); return }
    fetch(`${API_URL}/favorites/check/${song.id}`, { headers: authH(false) })
      .then(r => r.ok ? r.json() : null)
      .then(d => setFavState(d?.isFavorite ?? false))
      .catch(() => setFavState(false))
  }, [song.id, userId])
  ...
  const toggleFav = async () => {
    if (!userId || !getToken()) { setToast("Login to favourite"); setTimeout(() => { setToast(""); onClose() }, 1400); return }
    setFavLoading(true)
    try {
      if (favState) {
        await fetch(`${API_URL}/favorites/${song.id}`, { method: "DELETE", headers: authH(false) })
        setFavState(false)
    ...
    setFavLoading(false)
    setTimeout(() => { setToast(""); onClose() }, 1300)
  }

  const menuItem = (icon, label, onClick, accent) => (
    <div
      onClick={onClick}
      style={{ display: "flex", alignItems: "center", gap: 11, padding: "11px 16px", cursor: "pointer", fontSize: 13, color: accent ? "var(--app-accent)" : "var(--app-text-main)", transition: "background 0.15s", userSelect: "none" }}
      onMouseEnter={e => e.currentTarget.style.background = "rgba(var(--app-accent-rgb),0.09)"}
      onMouseLeave={e => e.currentTarget.style.background = "transparent"}
    >
  ...
  return (
    <div ref={ref} style={{
      position: "absolute", zIndex: 2000, top: "110%", right: 0,
      background: "var(--app-shell-bg-alt)", border: "1px solid rgba(var(--app-accent-rgb),0.22)",
      borderRadius: 12, minWidth: 188, boxShadow: "0 16px 48px rgba(0,0,0,0.85)",
      backdropFilter: "blur(16px)", overflow: "hidden",
    }}>
      {toast
        ? <div style={{ padding: "14px 16px", textAlign: "center", color: "var(--app-accent)", fontSize: 13, fontWeight: 600 }}>{toast}</div>
        : <>
            {menuItem(
              favState === null ? "…" : favState ? "💔" : "♥",
              favLoading ? "Please wait…" : favState ? "Remove from Favourites" : "Add to Favourites",
              toggleFav,
              true
            )}
          </>
...
// ── Mobile Action Sheet — bottom drawer with Playlist + Favourite ─────────────
function MobileActionSheet({ song, userId, onClose }) {
  const [phase, setPhase] = useState("menu") // "menu" | "playlist"
  const [playlists, setPlaylists] = useState([])
  ...
  const openPlaylist = () => {
    ...
    fetch(`${API_URL}/playlists`, { headers: authH(false) })
      .then(r => r.json()).then(d => { if (d.success) setPlaylists(d.playlists) })
      .catch(console.error).finally(() => setPlLoading(false))
  }
  ...
  const sheetItem = (icon, label, onClick, accent, rightEl) => (
    <div
      onClick={onClick}
      style={{ display: "flex", alignItems: "center", gap: 14, padding: "15px 20px", cursor: "pointer", fontSize: 14.5, color: accent ? "var(--app-accent)" : "var(--app-text-main)", borderBottom: "1px solid rgba(var(--app-accent-rgb),0.07)", userSelect: "none", WebkitTapHighlightColor: "transparent" }}
      onTouchStart={e => e.currentTarget.style.background = "rgba(var(--app-accent-rgb),0.09)"}
      onTouchEnd={e => { e.currentTarget.style.background = "transparent"; onClick && onClick() }}
    >
  ...
  return (
    <div
      ...
      style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}
    >
      <div
        ref={sheetRef}
        style={{
          background: "var(--app-shell-bg-alt)", borderRadius: "20px 20px 0 0",
          border: "1px solid rgba(var(--app-accent-rgb),0.18)", borderBottom: "none",
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "center", padding: "12px 0 4px" }}>
          <div style={{ width: 36, height: 4, borderRadius: 2, background: "rgba(var(--app-accent-rgb),0.25)" }} />
        </div>

        {/* Song info header */}
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 20px 14px", borderBottom: "1px solid rgba(var(--app-accent-rgb),0.1)" }}>
          <div style={{ width: 46, height: 46, borderRadius: 9, overflow: "hidden", background: "var(--app-surface)", flexShrink: 0 }}>
            {song.cover_url ? <img src={song.cover_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>🎵</div>}
          </div>
          <div style={{ minWidth: 0 }}>
            <div style={{ color: "var(--app-text-main)", fontWeight: 700, fontSize: 14, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{song.name}</div>
            <div style={{ color: "var(--app-text-muted)", fontSize: 12, marginTop: 2 }}>{song.artist}</div>
          </div>
        </div>

        {toast
          ? <div style={{ padding: "22px 20px", textAlign: "center", color: "var(--app-accent)", fontSize: 15, fontWeight: 700 }}>{toast}</div>
          : phase === "menu"
          ? <>
              {sheetItem(
                favState === null ? "…" : favState ? "💔" : "♥",
              ...
              {sheetItem("📋", "Add to Playlist", openPlaylist, false, <span style={{ color: "var(--app-text-muted)", fontSize: 16 }}>›</span>)}
              <div onClick={onClose} style={{ padding: "15px 20px", textAlign: "center", color: "var(--app-text-muted)", fontSize: 14, cursor: "pointer", fontWeight: 500 }}>Cancel</div>
            </>
          : <>
              {/* Back header */}
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px 8px", borderBottom: "1px solid rgba(var(--app-accent-rgb),0.08)" }}>
                <button onClick={() => setPhase("menu")} style={{ background: "none", border: "none", color: "var(--app-accent)", fontSize: 22, cursor: "pointer", padding: "2px 6px", borderRadius: 6 }}>‹</button>
                <span style={{ color: "var(--app-text-main)", fontWeight: 700, fontSize: 13, letterSpacing: "0.05em" }}>📋 SELECT PLAYLIST</span>
              </div>
              {plLoading
                ? <div style={{ padding: "20px", color: "var(--app-text-muted)", fontSize: 13, textAlign: "center" }}>Loading…</div>
                : playlists.length === 0
                ? <div style={{ padding: "20px", color: "var(--app-text-muted)", fontSize: 13, textAlign: "center" }}>No playlists found</div>
                : playlists.map(pl => (
                  <div key={pl._id}
                    onTouchEnd={e => { e.currentTarget.style.background = "transparent"; addToPlaylist(pl) }}
                    onTouchStart={e => e.currentTarget.style.background = "rgba(var(--app-accent-rgb),0.09)"}
                    onClick={() => addToPlaylist(pl)}
                    style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 20px", cursor: "pointer", borderBottom: "1px solid rgba(var(--app-accent-rgb),0.06)", color: adding === pl._id ? "var(--app-accent)" : "var(--app-text-main)", fontSize: 14 }}>
                    <span>📋</span>
                    <span style={{ flex: 1 }}>{pl.name}</span>
                    {adding === pl._id && <span style={{ fontSize: 12, color: "var(--app-accent)" }}>Adding…</span>}
                  </div>
                ))
              }
              <div onClick={onClose} style={{ padding: "15px 20px", textAlign: "center", color: "var(--app-text-muted)", fontSize: 14, cursor: "pointer", fontWeight: 500 }}>Cancel</div>
...
export default function ArtistPage() {
  ...
  const [openPlusId, setOpenPlusId] = useState(null)       // desktop + dropdown
  const [openDotsId, setOpenDotsId] = useState(null)       // desktop ⋮ context menu
  const [mobileSheetSong, setMobileSheetSong] = useState(null) // mobile bottom sheet
  ...
  const closeAll = useCallback(() => { setOpenPlusId(null); setOpenDotsId(null) }, [])
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: "var(--app-shell-bg)", color: "var(--app-text-main)", fontFamily: "'DM Sans',sans-serif", overflow: "hidden" }}>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        /* Desktop: show all action columns; Mobile: collapse to single ⋮ */
        .song-actions-desktop{display:flex;align-items:center;gap:6px;flex-shrink:0}
        .song-actions-mobile{display:none;align-items:center;flex-shrink:0}
        .song-duration-col{color:var(--app-text-muted);font-size:12.5px;font-family:monospace;flex-shrink:0}

        @media(max-width:540px){
          .qa-song-row{grid-template-columns:24px 38px 1fr auto;gap:8px;padding:8px 8px}
          .song-actions-desktop{display:none}
          .song-actions-mobile{display:flex}
          .song-duration-col{display:none}
        }
        ...
        /* ── Artist portrait card in hero (top-left) ── */
        .hero-artist-portrait{
          position:absolute;top:16px;left:16px;z-index:4;
          display:flex;flex-direction:column;align-items:center;gap:6px;
          animation:fadeIn 0.6s ease 0.1s both;
        }
        .hero-artist-portrait-img{
          width:72px;height:96px;border-radius:10px;overflow:hidden;
          border:2px solid rgba(var(--app-accent-rgb),0.55);
          box-shadow:0 4px 20px rgba(0,0,0,0.7),0 0 0 1px rgba(var(--app-accent-rgb),0.15);
        ...
        .hero-artist-portrait-label{
          font-size:9.5px;font-weight:700;color:rgba(255,255,255,0.55);letter-spacing:0.1em;
          text-transform:uppercase;text-align:center;max-width:80px;
          white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 1px 6px rgba(0,0,0,0.9);
        }

        @media(max-width:600px){
          .hero-artist-portrait-img{width:56px;height:74px;border-radius:8px}
          .hero-hero-content{padding:0 14px 24px !important;padding-left:84px !important}
        ...
        @media(max-width:400px){
          .hero-artist-portrait-img{width:46px;height:62px}
          .hero-hero-content{padding-left:68px !important}
        }
        @media(max-width:600px){
        ...
        .mob-dots-btn{
          width:32px;height:32px;border-radius:50%;background:rgba(var(--app-accent-rgb),0.08);
          border:1px solid rgba(var(--app-accent-rgb),0.22);color:var(--app-text-muted);
      ...
      {mobileSheetSong && (
        <MobileActionSheet
          song={mobileSheetSong}
          userId={userId}
          onClose={() => setMobileSheetSong(null)}
        />
      )}
          ...
          <div style={{ flex: 1, overflowY: "auto", position: "relative", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ position: "relative", height: 380, overflow: "hidden", flexShrink: 0 }}>
              ...
              <div className="hero-hero-content"
                style={{ position: "absolute", inset: 0, zIndex: 3, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "0 32px 36px", paddingLeft: 108 }}>
                ...
                <h1 className="fade-up fade-up-1 hero-title"
                  style={{ fontSize: "clamp(28px,4.5vw,72px)", fontWeight: 900, color: "#fff", lineHeight: 1, letterSpacing: "-0.03em", margin: "0 0 8px", textShadow: "0 2px 32px rgba(0,0,0,0.7)" }}>
                  {decodedName}
                </h1>
                    ...
                    : sortedSongs.map((song, idx) => {
                        ...
                        return (
                          <div key={song.id} className={`qa-song-row${active ? " active-row" : ""}`}>
                            ...
                            {/* Cover */}
                            <div onClick={() => playSongFromList(song)}
                              style={{ width: 44, height: 44, borderRadius: 9, overflow: "hidden", background: "var(--app-surface)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, cursor: "pointer", position: "relative", flexShrink: 0, boxShadow: active ? "0 0 14px rgba(var(--app-accent-rgb),0.35)" : "none" }}>
                            ...
                            <div className="song-duration-col">{fmt(song.duration)}</div>

                            {/* ── DESKTOP ACTIONS: FavouriteButton + + dropdown + ⋮ context menu ── */}
                            <div className="song-actions-desktop">
                              <FavoriteButton song={song} />
                              <div style={{ position: "relative" }}>
                                <button
                                  onClick={e => { e.stopPropagation(); setOpenDotsId(null); setOpenPlusId(openPlusId === song.id ? null : song.id) }}
                              ...
                              <div style={{ position: "relative" }}>
                                <button
                                  onClick={e => { e.stopPropagation(); setOpenPlusId(null); setOpenDotsId(openDotsId === song.id ? null : song.id) }}
                                  style={{ width: 28, height: 28, borderRadius: "50%", background: "transparent", border: "none", color: "var(--app-text-muted)", fontSize: 17, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "color 0.15s" }}
                                  onMouseEnter={e => e.currentTarget.style.color = "var(--app-text-main)"}
                                  onMouseLeave={e => e.currentTarget.style.color = "var(--app-text-muted)"}
                                  title="More options"
                                >⋮</button>
                                {openDotsId === song.id && (
                                  <SongContextMenu song={song} userId={userId} onClose={() => setOpenDotsId(null)} />
                                )}
                              </div>
                            </div>

                            {/* ── MOBILE ACTIONS: single ⋮ → bottom sheet ── */}
                            <div className="song-actions-mobile">
                              <button
                                className="mob-dots-btn"
                                onClick={e => { e.stopPropagation(); setMobileSheetSong(song) }}
                              >⋮</button>
                            </div>
                          </div>
                        )
      ...
      <div className="player-bar">
        ...
        <div className="player-controls">
          <button onClick={playPrev} style={{ background: "none", border: "none", color: "var(--app-text-muted)", cursor: "pointer", fontSize: 18, padding: 4 }}
          ...
          >{isPlaying ? "⏸" : "▶"}</button>
          <button onClick={playNext} style={{ background: "none", border: "none", color: "var(--app-text-muted)", cursor: "pointer", fontSize: 18, padding: 4 }}
            onMouseEnter={e => e.currentTarget.style.color = "var(--app-text-main)"} onMouseLeave={e => e.currentTarget.style.color = "var(--app-text-muted)"}>⏭</button>
