...
function TopArtists({ navigate, currentArtist }) {
  ...
  useEffect(() => {
    let cancelled = false
    fetch(
      `${SUPABASE_URL}/rest/v1/artists?select=name,image_url&order=name.asc`,
      { headers: H }
    )
      .then((r) => r.json())
      .then(async (rows) => {
        ...
        const songsRes = await fetch(
          `${SUPABASE_URL}/rest/v1/songs?select=artist&artist=not.is.null`,
          { headers: H }
        )
        ...
        // ✅ Fixed: include artists even without image_url, use dicebear fallback
        const all = rows
          .filter((r) => r.name)
          .map((r) => ({
            name: r.name,
        ...
        ? Array.from({ length: 4 }).map((_, i) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div
                  className="qa-skeleton"
                  style={{ width: "70%", height: 10, borderRadius: 4, marginBottom: 5 }}
                />
                <div
                  className="qa-skeleton"
        ...
        : artists.map((a) => {
            ...
            return (
              <div
                key={a.name}
                onClick={() =>
                  navigate(`/hero/artist/${encodeURIComponent(a.name)}`)
                }
                style={{
                  display: "flex",
                  alignItems: "center",
                  ...
                  background: isActive
                    ...
                    : "transparent",
                  border: isActive
                    ? "1px solid rgba(var(--app-accent-rgb),0.2)"
                    : "1px solid transparent",
                  transition: "all 0.15s",
                }}
                onMouseEnter={(e) => {
                  if (!isActive) e.currentTarget.style.background = "var(--app-surface)"
                }}
                onMouseLeave={(e) => {
              ...
              >
                <div
                  style={{
                    ...
                    flexShrink: 0,
                    border: `2px solid ${
                      isActive ? "var(--app-accent)" : "rgba(var(--app-accent-rgb),0.2)"
                    }`,
                    background: "var(--app-surface)",
                  }}
                >
                  <img
                    src={a.imageUrl || dicebearUrl(a.name)}
                    alt={a.name}
                    style={{ width: "100%", height: "100%", objectFit: "cover" }}
                    onError={(e) => {
                      e.target.src = dicebearUrl(a.name)
                    }}
                  />
                </div>
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div
                    style={{
                      color: isActive ? "var(--app-accent)" : "var(--app-text-main)",
                      fontWeight: 600,
                      fontSize: 12.5,
                      whiteSpace: "nowrap",
                      overflow: "hidden",
                  ...
                  <div
                    style={{ color: "var(--app-text-muted)", fontSize: 10.5, marginTop: 1 }}
                  >
                    {a.count} song{a.count !== 1 ? "s" : ""}
                  </div>
                </div>
                {isActive && (
                  <span style={{ color: "var(--app-accent)", fontSize: 10, flexShrink: 0 }}>
                    ●
                  </span>
                )}
              </div>
  ...
  return (
    ...
    >
      ...
      {toast ? (
        ...
        <div
          style={{ padding: 14, color: "var(--app-text-muted)", fontSize: 12, textAlign: "center" }}
        >
          Loading…
        </div>
      ) : playlists.length === 0 ? (
        ...
        playlists.map((pl) => (
          <div
            ...
            onMouseEnter={(e) =>
              (e.currentTarget.style.background = "rgba(var(--app-accent-rgb),0.1)")
            }
          ...
          >
            ...
            {adding === pl._id && (
              <span style={{ fontSize: 11, color: "var(--app-accent)" }}>Adding…</span>
            )}
          </div>
        ))
      )}
    ...
    },
    {
      ...
      action: () => {
        navigator.clipboard?.writeText(
          `${window.location.origin}/hero/artist/${encodeURIComponent(song.artist)}`
        )
        onClose()
      },
    },
    ...
    <div className={`qa-song-row${active ? " active-row" : ""}`}>
      ...
      <div
        ...
        style={{
          ...
          position: "relative",
          flexShrink: 0,
          boxShadow: active ? "0 0 14px rgba(var(--app-accent-rgb),0.35)" : "none",
        }}
      >
        {song.cover_url ? (
          <img
      ...
      >
        ...
        >
          ...
          {song.location && (
            <span style={{ color: "var(--app-text-muted)", fontSize: 11.5 }}>
              {song.location}
            </span>
          )}
        </div>
      ...
      >
        ...
        <div style={{ position: "relative" }}>
          <button
            ...
            style={{
              ...
              height: 28,
              borderRadius: "50%",
              background: isPlusOpen
                ? "rgba(var(--app-accent-rgb),0.25)"
                : "rgba(var(--app-accent-rgb),0.1)",
            ...
            onMouseEnter={(e) =>
              (e.currentTarget.style.background = "rgba(var(--app-accent-rgb),0.25)")
            }
            onMouseLeave={(e) => {
              if (!isPlusOpen)
                e.currentTarget.style.background = "rgba(var(--app-accent-rgb),0.1)"
        ...
        <div style={{ position: "relative" }}>
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.color = "var(--app-text-main)"
...
export default function ArtistPage() {
  ...
  // ✅ useNavbar called once here — no duplicate renders
  ...
  useEffect(() => {
    ...
    fetch(
      `${SUPABASE_URL}/rest/v1/artists?select=*&name=ilike.${encodeURIComponent(decodedName)}&limit=1`,
      { headers: H }
    )
      .then((r) => r.json())
  ...
  useEffect(() => {
    if (!decodedName) return
    fetch(`${API_URL}/artists/by-name/${encodeURIComponent(decodedName)}`, {
      headers: authH(false),
    })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (d?.success) setMongoArtist(d.artist)
      })
      .catch(() => {})
  }, [decodedName])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    fetch(
      `${SUPABASE_URL}/rest/v1/songs?select=*&artist=ilike.*${encodeURIComponent(decodedName)}*&order=created_at.desc`,
      { headers: H }
    )
      .then((r) => r.json())
  ...
  const playAll = () => {
    if (songs.length > 0) playSongFromList(songs[0])
  }
  const shufflePlay = () => {
    if (songs.length > 0)
      playSongFromList(songs[Math.floor(Math.random() * songs.length)])
  }
  ...
  return (
    ...
    >
      <link
        href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,400;0,9..40,500;0,9..40,600;0,9..40,700;0,9..40,800;0,9..40,900;1,9..40,400;1,9..40,600&family=Dancing+Script:wght@700&display=swap"
        rel="stylesheet"
      />
      <style>{`
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden", minWidth: 0 }}>
            ...
            >
              ...
              <div
                style={{
                  ...
                  zIndex: 1,
                  background:
                    "linear-gradient(to right, #0a0a0a 0%, #0a0a0a 18%, rgba(10,10,10,0.88) 28%, rgba(10,10,10,0.45) 40%, rgba(10,10,10,0.1) 55%, transparent 68%)",
                }}
              />
              <div
                style={{
                  ...
                  right: 0,
                  height: 110,
                  background: "linear-gradient(to bottom, transparent, var(--app-shell-bg))",
                  zIndex: 2,
                }}
              />
              ...
              <div className="hero-artist-portrait">
                <div className="hero-artist-portrait-img">
                  <img
                    ...
                    alt={decodedName}
                    onError={(e) => {
                      e.target.src = dicebearUrl(decodedName)
                    }}
                  />
                </div>
              ...
              >
                ...
                <h1
                  className="fade-up fade-up-1 hero-title"
                  style={{
                ...
                <div
                  className="fade-up fade-up-2 hero-stats"
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: 28,
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <div>
                      <div
                        style={{ color: "#fff", fontWeight: 800, fontSize: 14, lineHeight: 1.1 }}
                      >
                ...
                >
                  ...
                  <button className="qa-btn qa-btn-outline qa-btn-outline-hide">
                ...
                <div style={{ padding: "18px 16px 80px" }}>
                  ...
                  >
                    <div style={{ color: "var(--app-text-main)", fontSize: 13.5, fontWeight: 700 }}>
                      All Songs{" "}
                      <span style={{ color: "var(--app-text-muted)", fontWeight: 500 }}>
                        ({songs.length})
                      </span>
                    </div>
                    <div
                      className="songs-header-right"
                    ...
                    >
                      ...
                      <span style={{ color: "var(--app-text-muted)", fontSize: 17, cursor: "pointer" }}>
                        ☰
                      </span>
                    </div>
                  </div>
                  ...
                  {loading
                    ? Array.from({ length: 5 }).map((_, i) => (
                        <div
                          key={i}
                          className="qa-skeleton"
                          style={{ height: 54, borderRadius: 10, marginBottom: 4 }}
                        />
                      ))
          ...
          <div className="qa-right-panel">
            <div>
              ...
              {bio ? (
                <p style={{ color: "var(--app-text-muted)", fontSize: 12, lineHeight: 1.75 }}>
                  {bio}
                </p>
              ) : (
            ...
            {monthlyListeners && (
              <div className="qa-stat-card">
                ...
                <div>
                  <div style={{ color: "var(--app-text-main)", fontWeight: 800, fontSize: 15.5 }}>
                    {monthlyListeners}
                  </div>
                  <div style={{ color: "var(--app-text-muted)", fontSize: 11 }}>
                    Monthly Listeners
                  </div>
                </div>
              </div>
            ...
            {quote && (
              ...
              >
                <div
                  style={{ color: "var(--app-accent)", fontSize: 22, lineHeight: 1, marginBottom: 8 }}
                >
                  "
                </div>
                <p
            ...
            {currentSong && (
              ...
              >
                ...
                <div
                  style={{ color: "var(--app-text-muted)", fontSize: 11, marginBottom: 10 }}
                >
                  {currentSong.artist}
                </div>
