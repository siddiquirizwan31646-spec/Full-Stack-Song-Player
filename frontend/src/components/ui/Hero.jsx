      ...
      >
        ...
        <div
          style={{
            ...
            height: 1,
            background:
              "linear-gradient(to right,rgba(var(--app-accent-rgb),0.2),transparent)",
          }}
        />
    ...
    >
      <div
        ...
        onMouseEnter={(e) => {
          ...
          e.currentTarget.style.boxShadow =
            "0 4px 22px rgba(var(--app-accent-rgb),0.35)"
        }}
        onMouseLeave={(e) => {
      ...
      >
        {!loaded && (
          <div
            style={{
              ...
              inset: 0,
              background:
                "linear-gradient(90deg,var(--app-surface) 25%,rgba(var(--app-accent-rgb),0.06) 50%,var(--app-surface) 75%)",
        ...
        <img
          ...
          onError={(e) => {
            e.target.src = dicebearUrl(artistName)
            setLoaded(true)
          }}
          style={{
            width: "100%",
  ...
  useEffect(() => {
    ...
    fetch(artistsUrl, { headers: H })
      ...
      .then(async (artistRows) => {
        // If no artists table rows, fall back to deriving from songs
        ...
        // Build artist map — include ALL artists even without image_url
        // Artists without image_url will use dicebear fallback in ArtistCard
        const map = {}

        // First add artists from artists table
        rows.forEach((r) => {
          if (r.name) {
            map[r.name] = {
        ...
        // Also add artists that appear in songs but may not be in artists table
        if (Array.isArray(songRows)) {
          songRows.forEach((r) => {
            const a = (r.artist || "").trim()
            if (a && !map[a]) {
              map[a] = {
                imageUrl: null,
                count: countMap[a.toLowerCase()] || 0,
              }
            }
          })
        }
        ...
        {loading
          ? Array.from({ length: 8 }).map((_, i) => (
              ...
              >
                ...
                <div
                  className="skeleton"
                  style={{ width: 70, height: 10, borderRadius: 4 }}
                />
              </div>
            ))
          : displayed.map((name) => (
              <ArtistCard
                ...
                onClick={(n) =>
                  navigate(`/hero/artist/${encodeURIComponent(n)}`)
                }
              />
            ))}
      </div>
...
function AddToPlaylistDropdown({ song, userId, onClose, tr }) {
  ...
  const add = async (pl) => {
    ...
    try {
      ...
      setToast(
        r.status === 409 ? tr.alreadyIn : r.ok ? tr.added : tr.failed
      )
    } catch {
      setToast(tr.error)
    }
      ...
      <button
        ...
        style={{
          ...
          background: open
            ...
            : "rgba(var(--app-accent-rgb),0.1)",
          border: `1px solid ${
            open ? "var(--app-accent)" : "rgba(var(--app-accent-rgb),0.3)"
          }`,
          color: "var(--app-accent)",
          fontSize: 18,
          cursor: "pointer",
      ...
      >
        <div
          ...
          style={{
            ...
            transition: "all 0.2s",
            boxShadow: isActive
              ? "0 4px 20px rgba(var(--app-accent-rgb),0.15)"
              : "none",
          }}
        >
          <div
    ...
    <div style={{ position: "relative", width: 138, flexShrink: 0 }}>
      <div
        ...
        style={{
          ...
          transition: "all 0.2s",
          boxShadow: isActive
            ? "0 6px 24px rgba(var(--app-accent-rgb),0.2)"
          ...
          >
            <div
              style={{
                ...
                alignItems: "center",
                justifyContent: "center",
                color: isActive ? "#000" : "var(--app-accent)",
                fontSize: 8,
                transition: "all 0.2s",
              }}
            >
...
export default function QalbAudio() {
  ...
  const fetchSongs = useCallback(async (q = "", p = 0, append = false) => {
    ...
    try {
      ...
      if (q)
        url += `&or=(name.ilike.*${encodeURIComponent(q)}*,artist.ilike.*${encodeURIComponent(q)}*)`
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        {/* Sidebar — single render, no duplicate */}
        ...
        >
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
                ...
                {loading
                  ? Array.from({ length: 4 }).map((_, i) => (
                      <div
                        key={i}
                        className="skeleton"
                        style={{ borderRadius: 14, height: 78 }}
                      />
                    ))
            ...
            >
              {[
                { label: tr.recentlyPlayed, sl: [0, 5] },
                { label: tr.trending, sl: [5, 10] },
              ].map(({ label, sl }) => (
                <Section key={label} title={label}>
                  <div className="h-scroll">
                    {(loading ? Array.from({ length: 4 }) : songs.slice(...sl)).map(
                      (s, i) =>
                        s ? (
                          <SongCard
                            key={s.id}
            ...
            >
              ...
              <div
                style={{
                  ...
                  height: 1,
                  background:
                    "linear-gradient(to right,var(--app-border),transparent)",
                }}
              />
            </div>
            ...
            {songs.map((song) => {
              ...
              return (
                <div
                  key={song.id}
                  className={`song-row${active ? " active-row" : ""}`}
                >
                  <div
                  ...
                  >
                    <div
                      style={{
                        ...
                        boxShadow: active
                          ? "0 0 14px rgba(var(--app-accent-rgb),0.35)"
                          : "none",
                      }}
                    >
                      {song.cover_url ? (
        ...
        <div className="right-panel">
          <div>
            ...
            >
              {[
                { label: tr.totalSongs, value: total || 0 },
                { label: tr.listeners, value: "5.8K" },
                { label: tr.saves, value: "940" },
                { label: tr.likes, value: "11.2K" },
              ].map((s) => (
                <div
                  key={s.label}
                  style={{
                ...
                >
                  <div
                    style={{ color: "var(--app-text-muted)", fontSize: 10 }}
                  >
                    {s.label}
                  </div>
                  <div
          ...
          <div>
            ...
            >
              ...
              >
                <polyline
                  points="0,50 20,36 40,40 60,26 80,33 100,18 120,28 140,16 160,23 180,13"
                  fill="none"
                ...
                />
                <polyline
                  points="0,50 20,36 40,40 60,26 80,33 100,18 120,28 140,16 160,23 180,13 180,56 0,56"
                  fill="rgba(var(--app-accent-rgb),0.08)"
                />
              </svg>
            </div>
          </div>
          ...
          <div
            style={{
              ...
              boxShadow: currentSong
                ? "0 0 12px rgba(var(--app-accent-rgb),0.25)"
                : "none",
              position: "relative",
            }}
          >
        ...
        <div className="player-controls">
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              flexShrink: 0,
              boxShadow: "0 4px 14px rgba(var(--app-accent-rgb),0.4)",
