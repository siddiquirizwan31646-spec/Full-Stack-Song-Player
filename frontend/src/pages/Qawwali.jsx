...
export default function QawwaliPage() {
  ...
  const fetchQawwalis = useCallback(async (q = "", p = 0, append = false) => {
    setLoading(true);
    try {
      const from = p * 20;
      let url = `${SUPABASE_URL}/rest/v1/songs?select=*&music_type=eq.qawwali&order=created_at.desc&limit=20&offset=${from}`;
  ...
  useEffect(() => { fetchQawwalis(); }, [fetchQawwalis]);

  const handleSearch = (val) => {
    setSearch(val); setPage(0);
    clearTimeout(searchTimer.current);
    searchTimer.current = setTimeout(() => fetchQawwalis(val, 0, false), 400);
  };
    ...
    <div style={{
      ...
      <style>{`
        *{box-sizing:border-box}
        ...
        /* HERO BANNER */
        .hero-banner{margin:16px 16px 0;background:linear-gradient(135deg,rgba(var(--app-accent-rgb),0.12) 0%,rgba(180,83,9,0.08) 50%,rgba(120,53,15,0.06) 100%);border:1px solid rgba(var(--app-accent-rgb),0.2);border-radius:14px;padding:18px 20px;display:flex;align-items:center;justify-content:space-between}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div className={`sidebar${sidebarOpen ? " open" : ""}`}>
          ...
          <nav style={{ flex: 1, padding: "10px 8px", overflowY: "auto" }}>
            {NAV_ITEMS.map((item) => (
              <div key={item.id}
                className={`nav-item${item.id === "qawwali" ? " active" : ""}`}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "var(--app-shell-bg-alt)", borderBottom: "1px solid rgba(var(--app-accent-rgb),0.08)", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ color: "var(--app-text-main)", fontSize: 16, fontWeight: 700 }}>Qawwali</span>
            ...
            <div className="topbar-search" style={{ position: "relative" }}>
              ...
              <input
                ...
                placeholder="Search qawwalis, artists..."
                style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 0 8px" }}>
            ...
            <div className="hero-banner">
              <div>
                <div style={{ color: "var(--app-accent)", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", marginBottom: 5 }}>DEVOTIONAL SUFI MUSIC</div>
                <div style={{ color: "var(--app-text-main)", fontSize: "clamp(16px,3vw,22px)", fontWeight: 800, marginBottom: 4 }}>Qawwali — The Sound of the Soul</div>
                <div style={{ color: "var(--app-text-muted)", fontSize: 12 }}>Classical & contemporary qawwalis from legendary artists</div>
              </div>
            ...
            <div style={{ padding: "14px 16px 0" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <span style={{ color: "var(--app-text-muted)", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", flexShrink: 0 }}>
                  {loading ? "LOADING..." : `ALL QAWWALIS · ${total}`}
                </span>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--app-text-muted)" }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 600 }}>No qawwalis found</div>
              ...
              {songs.length < total && (
                <button
                  onClick={() => { const np = page + 1; setPage(np); fetchQawwalis(search, np, true); }}
                  style={{
