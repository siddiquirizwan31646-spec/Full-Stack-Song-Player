...
const ChangePassword = () => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      setIsLoading(true)
      const res = await axios.post(`${API_URL}/user/change-password/${encodeURIComponent(normalizedEmail)}`, {
        newPassword,
        confirmPassword,
      })
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "fixed", top: -100, right: -100, width: 360, height: 360, borderRadius: "50%", background: "radial-gradient(circle,rgba(var(--app-accent-rgb,34,197,94),0.1) 0%,transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "fixed", bottom: -80, left: -80, width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle,rgba(var(--app-accent-rgb,34,197,94),0.07) 0%,transparent 70%)", pointerEvents: "none" }} />
      ...
      >
        ...
        <h2 style={{ fontSize: 22, fontWeight: 800, color: "var(--app-text-main,#f0f0f0)", textAlign: "center", margin: "0 0 6px" }}>
          Change Password
        </h2>
        <p style={{ fontSize: 13, color: "var(--app-text-muted,#6b7280)", textAlign: "center", margin: "0 0 22px", lineHeight: 1.6 }}>
          Set a new password for{" "}
          <span style={{ color: "var(--app-accent,#22c55e)", fontWeight: 600 }}>{normalizedEmail}</span>
        </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            ...
            <Input type="password" placeholder="Enter new password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="cp-input" />
          </div>
          <div>
            ...
            <Input type="password" placeholder="Confirm new password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="cp-input" />
          </div>

          <button className="cp-btn" onClick={handleChangePassword} disabled={isLoading} style={{ marginTop: 4 }}>
            {isLoading ? <><Loader2 size={16} className="animate-spin" /> Changing Password...</> : "Change Password"}
          </button>
