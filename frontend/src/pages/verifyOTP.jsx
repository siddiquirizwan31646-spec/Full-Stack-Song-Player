...
export default function VerifyOTP() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/user/verify-otp/${email}`, { otp: code })
      setSuccessMsg(res.data.message); setVerified(true)
  ...
  const clearOtp = () => { setOtp(["","","","","",""]); setError(""); refs.current[0]?.focus() }
        ...
        <div style={{ background:"var(--app-surface)", backdropFilter:"blur(20px)", border:"1px solid rgba(var(--app-accent-rgb),.15)", borderRadius:20, boxShadow:"0 8px 40px rgba(0,0,0,.5)", overflow:"hidden" }}>
          ...
          <div style={{ padding:"28px 24px 0", textAlign:"center" }}>
            ...
            <p style={{ fontSize:13, color:"var(--app-text-muted)" }}>
              {verified ? "Code verified! Redirecting..." : "We've sent a 6-digit code to your email"}
            </p>
          </div>
            ...
            {verified ? (
              <div style={{ padding:"16px 0", display:"flex", flexDirection:"column", alignItems:"center", gap:16 }}>
                ...
                <div style={{ textAlign:"center" }}>
                  ...
                  <p style={{ fontSize:13, color:"var(--app-text-muted)", margin:0 }}>Your email has been verified. Redirecting shortly.</p>
                </div>
              ...
              <>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", gap:6, marginBottom:20 }}>
                  {otp.map((digit, i) => (
                    <input key={i} type="text" inputMode="numeric" value={digit}
                      onChange={e => handleChange(i, e.target.value)}
                      onKeyDown={e => handleKeyDown(i, e)}
                      maxLength={1} ref={el => refs.current[i] = el}
                      style={{
                      ...
                      onBlur={e => e.target.style.borderColor=digit ? "rgba(var(--app-accent-rgb),.5)" : "rgba(var(--app-accent-rgb),.15)"}
                    />
                  ))}
                </div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  ...
                  <button onClick={clearOtp} disabled={loading} style={{
                    ...
                    onMouseEnter={e => { e.currentTarget.style.borderColor="rgba(var(--app-accent-rgb),.4)"; e.currentTarget.style.color="var(--app-text-main)" }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor="rgba(var(--app-accent-rgb),.15)"; e.currentTarget.style.color="var(--app-text-muted)" }}>
          ...
          <div style={{ padding:"16px 24px 24px", textAlign:"center" }}>
            <p style={{ fontSize:13, color:"rgba(229,231,235,.35)", margin:0 }}>
              Wrong email?{" "}
              <Link to="/ForgotPassword" style={{ color:"var(--app-accent)", fontWeight:700, textDecoration:"none" }}>Go back</Link>
            </p>
          </div>
        </div>
