...
export default function NaatPage() {
  ...
  const fetchNaats = useCallback(async (q = "", p = 0, append = false) => {
    ...
    try {
      const from = p * 20;
      let url = `${SUPABASE_URL}/rest/v1/songs?select=*&music_type=eq.naat&order=created_at.desc&limit=20&offset=${from}`;
  ...
  useEffect(() => { fetchNaats(); }, [fetchNaats]);
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div className={`sidebar${sidebarOpen ? " open" : ""}`}>
          ...
          <nav style={{ flex: 1, padding: "10px 8px", overflowY: "auto" }}>
            {NAV_ITEMS.map(item => (
              <div key={item.id}
                className={`nav-item${item.id === "naat" ? " active" : ""}`}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "var(--app-shell-bg-alt)", borderBottom: "1px solid rgba(var(--app-accent-rgb),0.08)", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ color: "var(--app-text-main)", fontSize: 16, fontWeight: 700 }}>Naat</span>
            ...
            <div className="topbar-search" style={{ position: "relative" }}>
              ...
              <input value={search} onChange={e => handleSearch(e.target.value)}
                placeholder="Search naats, reciters..."
                style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 0 8px" }}>
            ...
            <div className="hero-banner">
              <div>
                <div style={{ color: "var(--app-accent)", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", marginBottom: 5 }}>NOW PLAYING COLLECTION</div>
                <div style={{ color: "var(--app-text-main)", fontSize: "clamp(16px,3vw,22px)", fontWeight: 800, marginBottom: 4 }}>Naats — Praise of the Prophet ﷺ</div>
                <div style={{ color: "var(--app-text-muted)", fontSize: 12 }}>Soulful recitations in Urdu, Arabic & Punjabi</div>
              </div>
            ...
            <div style={{ padding: "16px 16px 0" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <span style={{ color: "var(--app-text-muted)", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", flexShrink: 0 }}>
                  {loading ? "LOADING..." : `ALL NAATS · ${total}`}
                </span>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--app-text-muted)" }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 600 }}>No naats found</div>
              ...
              {songs.length < total && (
                <button onClick={() => { const np = page + 1; setPage(np); fetchNaats(search, np, true); }}
                  style={{
