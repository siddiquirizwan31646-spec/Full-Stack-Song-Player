...
/* ── page ────────────────────────────────────────────────────────── */
export default function NasheedPage() {
  const { user } = useUser();
  ...
  const fetchSongs = useCallback(async (q = "", p = 0, append = false) => {
    ...
    try {
      const from = p * 20;
      let url = `${SUPABASE_URL}/rest/v1/songs?select=*&music_type=eq.nasheed&order=created_at.desc&limit=20&offset=${from}`;
    ...
    <div style={{
      ...
      <style>{`
        *{box-sizing:border-box}
        ...
        /* HERO BANNER */
        .hero-banner{margin:16px 16px 0;background:linear-gradient(135deg,rgba(var(--app-accent-rgb),0.12) 0%,rgba(139,92,246,0.08) 50%,rgba(217,119,6,0.06) 100%);border:1px solid rgba(var(--app-accent-rgb),0.2);border-radius:14px;padding:18px 20px;display:flex;align-items:center;justify-content:space-between}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div className={`sidebar${sidebarOpen ? " open" : ""}`}>
          ...
          <nav style={{ flex: 1, padding: "10px 8px", overflowY: "auto" }}>
            {NAV_ITEMS.map(item => (
              <div key={item.id}
                className={`nav-item${item.id === "nasheed" ? " active" : ""}`}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "var(--app-shell-bg-alt)", borderBottom: "1px solid rgba(var(--app-accent-rgb),0.08)", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ color: "var(--app-text-main)", fontSize: 16, fontWeight: 700 }}>Nasheed</span>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 0 8px" }}>
            ...
            <div className="hero-banner">
              <div>
                <div style={{ color: "var(--app-accent)", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", marginBottom: 5 }}>ISLAMIC VOCAL MUSIC</div>
                <div style={{ color: "var(--app-text-main)", fontSize: "clamp(16px,3vw,22px)", fontWeight: 800, marginBottom: 4 }}>Nasheed Collection</div>
                <div style={{ color: "var(--app-text-muted)", fontSize: 12 }}>
                  {loading ? "Loading…" : `${total} nasheeds · Pure vocals & spiritual melodies`}
                </div>
              </div>
              <div className="hero-banner-aside" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ color: "#d97706", fontSize: 14, fontFamily: "serif" }}>بِسْمِ اللَّهِ</span>
              </div>
            </div>
            ...
            <div style={{ padding: "16px 16px 0" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <span style={{ color: "var(--app-text-muted)", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", flexShrink: 0 }}>
                  {loading ? "LOADING..." : `ALL NASHEEDS · ${total}`}
                </span>
              ...
              /* EMPTY */
                ...
                <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--app-text-muted)" }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 600 }}>No Nasheeds Found</div>
                  <div style={{ fontSize: 13, marginTop: 6 }}>
                    {search ? `No results for "${search}"` : "Upload some nasheeds to get started"}
                  </div>
              ...
              /* LIST */
                ...
                <div>
                  {songs.map((song, idx) => {
                    ...
                    const progress = isActive && duration > 0 ? currentTime / duration : 0;
                    return (
                      <div key={song.id} onClick={() => playSong(song)} style={{
              ...
              {songs.length < total && !loading && (
                <button onClick={() => { const np = page + 1; setPage(np); fetchSongs(search, np, true); }} style={{
        ...
        <div className="player-track">
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ color: "var(--app-text-muted)", fontSize: 11, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 140 }}>
              {currentSong?.artist || "Pick a nasheed to play"}
            </div>
          </div>
        </div>
