...
const formatBytes = (v) => { if (!Number.isFinite(v) || v <= 0) return "0 B"; const u = ["B","KB","MB","GB"]; const p = Math.min(Math.floor(Math.log(v)/Math.log(1024)),u.length-1); const s = v/1024**p; return `${s.toFixed(s>=10||p===0?0:1)} ${u[p]}`; };
const stripExtension = (n="") => n.replace(/\.[^/.]+$/,"");
const getExtension = (n="") => n.split(".").pop()?.toLowerCase()||"";
const slugify = (v="") => String(v).normalize("NFKD").replace(/[^\w\s-]/g,"").trim().toLowerCase().replace(/[\s_-]+/g,"-").replace(/^-+|-+$/g,"");
const normalizeStem = (v="") => slugify(stripExtension(v));
const formatDateTime = (v) => { if(!v) return "Not available"; try { return new Intl.DateTimeFormat("en",{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit"}).format(new Date(v)); } catch { return String(v); } };
const formatDuration = (s) => { if(!Number.isFinite(s)||s<=0) return "0:00"; const w=Math.round(s),h=Math.floor(w/3600),m=Math.floor((w%3600)/60),sc=w%60; if(h>0) return `${h}:${String(m).padStart(2,"0")}:${String(sc).padStart(2,"0")}`; return `${m}:${String(sc).padStart(2,"0")}`; };
const normalizeMusicType = (v="") => { const n=slugify(v).replace(/-/g,""); if(n.includes("nasheed")) return "nasheed"; if(n.includes("naat")) return "naat"; if(n.includes("qawwali")) return "qawwali"; if(n.includes("podcast")) return "podcast"; if(n.includes("quran")) return "quran"; if(n.includes("dua")) return "dua"; if(n.includes("lecture")||n.includes("khutbah")) return "lecture"; return slugify(v)||"other"; };
const excelSerialToDate = (s) => new Date(Date.UTC(1899,11,30)+s*86400000);
const parseDurationToSeconds = (v) => { if(v==null||v==="") return 0; if(typeof v==="number") { if(v>0&&v<1) return Math.round(v*24*60*60); return Math.round(v); } const r=String(v).trim(); if(!r) return 0; if(/^\d+(\.\d+)?$/.test(r)) return Math.round(Number(r)); const p=r.split(":").map(Number); if(p.every(Number.isFinite)) { if(p.length===2) return p[0]*60+p[1]; if(p.length===3) return p[0]*3600+p[1]*60+p[2]; } return 0; };
const parseUploadDate = (v) => { if(!v&&v!==0) return new Date().toISOString().slice(0,10); if(v instanceof Date&&!Number.isNaN(v.getTime())) return v.toISOString().slice(0,10); if(typeof v==="number") { if(v>10000) { const d=excelSerialToDate(v); if(!Number.isNaN(d.getTime())) return d.toISOString().slice(0,10); } return new Date(v).toISOString().slice(0,10); } const d=new Date(String(v)); if(!Number.isNaN(d.getTime())) return d.toISOString().slice(0,10); return new Date().toISOString().slice(0,10); };
const normalizeHeaderKey = (v="") => String(v).trim().toLowerCase().replace(/[^a-z0-9]/g,"");
const pickField = (r,keys) => { for(const k of keys) { if(r[k]!=null&&r[k]!=="") return r[k]; } return ""; };
const normalizeSpreadsheetRow = (row,index) => { const rec=Object.fromEntries(Object.entries(row).map(([k,v])=>[normalizeHeaderKey(k),v])); return { rowIndex:index+2, filename:String(pickField(rec,["filename","file","basename","songfile","audiofile"])).trim(), stem:normalizeStem(String(pickField(rec,["filename","file","basename","songfile","audiofile"])).trim()), title:String(pickField(rec,["title","name","songtitle"])).trim(), artist:String(pickField(rec,["artist","reciter","singer"])).trim(), music_type:normalizeMusicType(pickField(rec,["type","musictype","category"])), description:String(pickField(rec,["description","desc","about"])).trim(), location:String(pickField(rec,["location","country","city"])).trim(), duration:parseDurationToSeconds(pickField(rec,["duration","length","runtime"])), date:parseUploadDate(pickField(rec,["date","uploaddate","publishdate"])), }; };
const buildFileMap = (files) => { const m=new Map(); files.forEach((f)=>m.set(normalizeStem(f.name),f)); return m; };
const getPublicUrl = (bucket,path) => supabase.storage.from(bucket).getPublicUrl(path).data.publicUrl;
const buildStoragePath = (prefix,type,stem,originalName) => `${prefix}/${slugify(type||"other")||"other"}/${stem}.${getExtension(originalName)}`;
const extractStoragePath = (url,bucket) => { if(!url) return null; const marker=`/storage/v1/object/public/${bucket}/`; const index=url.indexOf(marker); if(index===-1) return null; return decodeURIComponent(url.slice(index+marker.length)); };
const extractStemFromUrl = (url) => { if(!url) return ""; try { const p=new URL(url).pathname; return normalizeStem(p.split("/").pop()||""); } catch { return normalizeStem(url); } };
const buildDuplicateIndex = (songs) => { const filenameStems=new Set(),signatures=new Set(); songs.forEach((s)=>{ const stem=extractStemFromUrl(s.mp3_url); if(stem) filenameStems.add(stem); signatures.add([slugify(s.name),slugify(s.artist),normalizeMusicType(s.music_type)].filter(Boolean).join("|")); }); return { filenameStems,signatures }; };
const createSignature = ({title,artist,music_type}) => [slugify(title),slugify(artist),normalizeMusicType(music_type)].filter(Boolean).join("|");
    ...
    <div style={{
      ...
      <style>{`
        *{box-sizing:border-box}
        ...
        .gate-input{
          width:100%;background:var(--app-surface);
        ...
        .gate-btn{
          ...
          box-shadow:0 4px 22px rgba(var(--app-accent-rgb),0.35);
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <button className="gate-btn" onClick={handleLogin} disabled={loading} style={{ marginTop: 4 }}>
              {loading ? <><Loader2 size={16} className="animate-spin" /> Verifying…</> : "Enter Upload Center"}
            </button>

            <button onClick={() => navigate("/hero")} style={{ background: "none", border: "none", color: "var(--app-text-muted)", fontFamily: "'DM Sans',sans-serif", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, margin: "0 auto", transition: "color 0.2s", padding: 0 }}
...
function StatCard({ icon: Icon, label, value, hint, accent = false }) {
  return (
    <div style={{ background: "var(--app-surface)", border: "1px solid var(--app-border)", borderRadius: 12, padding: "16px 16px 14px", boxShadow: "var(--app-shadow)" }}>
      <div style={{ width: 40, height: 40, borderRadius: 12, display: "grid", placeItems: "center", background: accent ? "rgba(var(--app-accent-rgb),0.14)" : "rgba(148,163,184,0.12)", color: accent ? "var(--app-accent)" : "var(--app-text-muted)", marginBottom: 10 }}><Icon size={17} /></div>
...
function FolderDropArea({ title, subtitle, count, totalSize, icon: Icon, buttonLabel, onPick, disabled, isDragActive, isDragReject, rootProps, inputProps }) {
  return (
    <div {...rootProps} style={{ borderRadius: 12, border: isDragReject ? "1px solid rgba(248,113,113,0.4)" : isDragActive ? "1px solid rgba(var(--app-accent-rgb),0.36)" : "1px solid var(--app-border)", background: isDragActive ? "rgba(var(--app-accent-rgb),0.08)" : "var(--app-surface)", padding: 18, boxShadow: "var(--app-shadow)", transition: "all 0.22s" }}>
      <input {...inputProps} />
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 13, display: "grid", placeItems: "center", background: "rgba(var(--app-accent-rgb),0.12)", color: "var(--app-accent)", flexShrink: 0 }}><Icon size={18} /></div>
          <div>
        ...
        <button type="button" onClick={(e) => { e.stopPropagation(); onPick(); }} disabled={disabled} style={{ border: "1px solid rgba(var(--app-accent-rgb),0.24)", background: disabled ? "rgba(148,163,184,0.1)" : "rgba(var(--app-accent-rgb),0.1)", color: disabled ? "var(--app-text-muted)" : "var(--app-accent)", borderRadius: 999, padding: "9px 13px", cursor: disabled ? "not-allowed" : "pointer", fontWeight: 700, whiteSpace: "nowrap", fontSize: 13 }}>{buttonLabel}</button>
      </div>
      <div style={{ marginTop: 14, borderRadius: 14, border: "1px dashed rgba(var(--app-accent-rgb),0.22)", padding: "14px 14px", color: "var(--app-text-muted)", fontSize: 12, background: isDragActive ? "rgba(var(--app-accent-rgb),0.06)" : "rgba(255,255,255,0.02)" }}>
        {isDragReject ? "This drop does not match the expected file type." : isDragActive ? "Drop the folder contents here." : "Drag a folder here or use the picker button."}
      </div>
    </div>
  );
}

function EditSongModal({ song, onClose, onSave, saving }) {
  const [draft, setDraft] = useState({ name: "", artist: "", music_type: "", description: "", duration: 0, date: "", location: "" });
  useEffect(() => { if (song) setDraft({ name: song.name || "", artist: song.artist || "", music_type: song.music_type || "", description: song.description || "", duration: song.duration || 0, date: song.date || "", location: song.location || "" }); }, [song]);
  if (!song) return null;
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(3,7,4,0.76)", backdropFilter: "blur(14px)", zIndex: 300, display: "grid", placeItems: "center", padding: 16 }}>
      <div style={{ width: "100%", maxWidth: 600, borderRadius: 16, background: "var(--app-surface-solid)", border: "1px solid var(--app-border)", boxShadow: "0 30px 80px rgba(0,0,0,0.36)", padding: 22 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 18 }}>
          ...
          <button type="button" onClick={onClose} style={{ width: 36, height: 36, borderRadius: 10, border: "1px solid var(--app-border)", background: "transparent", color: "var(--app-text-muted)", cursor: "pointer", display: "grid", placeItems: "center" }}><X size={16} /></button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 12 }}>
          {[{ key: "name", label: "Title", type: "text" }, { key: "artist", label: "Artist", type: "text" }, { key: "music_type", label: "Type", type: "text" }, { key: "location", label: "Location", type: "text" }, { key: "duration", label: "Duration (seconds)", type: "number" }, { key: "date", label: "Upload date", type: "date" }].map((field) => (
            <label key={field.key} style={{ display: "grid", gap: 6 }}>
              <span style={{ fontSize: 11, color: "var(--app-text-muted)", fontWeight: 700 }}>{field.label}</span>
              <input type={field.type} value={draft[field.key]} onChange={(e) => setDraft((c) => ({ ...c, [field.key]: field.type === "number" ? Number(e.target.value) : e.target.value }))} style={{ width: "100%", background: "var(--app-surface)", border: "1px solid var(--app-border)", borderRadius: 12, padding: "10px 12px", color: "var(--app-text-main)", outline: "none", boxSizing: "border-box", fontSize: 13 }} />
            </label>
          ))}
        </div>
        <label style={{ display: "grid", gap: 6, marginTop: 12 }}>
          <span style={{ fontSize: 11, color: "var(--app-text-muted)", fontWeight: 700 }}>Description</span>
          <textarea value={draft.description} onChange={(e) => setDraft((c) => ({ ...c, description: e.target.value }))} rows={3} style={{ width: "100%", background: "var(--app-surface)", border: "1px solid var(--app-border)", borderRadius: 14, padding: "10px 12px", color: "var(--app-text-main)", resize: "vertical", outline: "none", boxSizing: "border-box", fontSize: 13 }} />
        </label>
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 18 }}>
          <button type="button" onClick={onClose} style={{ padding: "10px 14px", borderRadius: 999, border: "1px solid var(--app-border)", background: "transparent", color: "var(--app-text-muted)", cursor: "pointer", fontWeight: 700, fontSize: 13 }}>Cancel</button>
          <button type="button" disabled={saving} onClick={() => onSave(draft)} style={{ padding: "10px 16px", borderRadius: 999, border: "none", background: "linear-gradient(135deg,var(--app-accent-strong),var(--app-accent))", color: "#041307", cursor: saving ? "not-allowed" : "pointer", fontWeight: 800, fontSize: 13, boxShadow: "0 8px 20px rgba(var(--app-accent-rgb),0.24)" }}>{saving ? "Saving..." : "Save changes"}</button>
        </div>
      </div>
    </div>
  );
...
function UploadCenter({ navigate, displayName, onLock }) {
  ...
  const commitQueueItems = useCallback((nextOrUpdater) => {
    const next = typeof nextOrUpdater === "function" ? nextOrUpdater(queueItemsRef.current) : nextOrUpdater;
    queueItemsRef.current = next; setQueueItems(next); return next;
  }, []);

  const appendLog = useCallback((level, message) => {
    setLogs((cur) => trimLogs([{ id: `${Date.now()}-${Math.random().toString(16).slice(2, 8)}`, level, message, createdAt: new Date().toISOString() }, ...cur]));
  }, []);

  const fetchUploadedSongs = useCallback(async ({ silent = false } = {}) => {
    if (!silent) setSongsLoading(true);
    const { data, error } = await supabase.from(SONGS_TABLE).select("id,name,artist,music_type,description,duration,date,location,mp3_url,cover_url,created_at").order("created_at", { ascending: false }).range(0, 999);
    if (error) { if (!silent) setSongsLoading(false); throw error; }
  ...
  useEffect(() => {
    [songsInputRef, imagesInputRef, spreadsheetInputRef].forEach(({ current }) => {
      if (!current) return;
      current.setAttribute("webkitdirectory", ""); current.setAttribute("directory", ""); current.setAttribute("multiple", "");
    });
  }, []);

  useEffect(() => {
    let mounted = true;
    fetchUploadedSongs().then((rows) => { if (mounted) appendLog("info", `Loaded ${rows.length} existing song records.`); }).catch((e) => toast.error(e.message || "Unable to load songs."));
    return () => { mounted = false; };
  }, [appendLog, fetchUploadedSongs]);

  const totalSongSize = useMemo(() => formatBytes(songFiles.reduce((s, f) => s + f.size, 0)), [songFiles]);
  const totalImageSize = useMemo(() => formatBytes(imageFiles.reduce((s, f) => s + f.size, 0)), [imageFiles]);
  const totalSpreadsheetSize = useMemo(() => formatBytes(spreadsheetFiles.reduce((s, f) => s + f.size, 0)), [spreadsheetFiles]);

  const queueStats = useMemo(() => {
    const s = { total: queueItems.length, ready: 0, uploading: 0, success: 0, failed: 0, skipped: 0 };
    queueItems.forEach((i) => { s[i.status] += 1; }); return s;
  }, [queueItems]);
  ...
  const filteredSongs = useMemo(() => {
    ...
    return uploadedSongs.filter((s) => {
      ...
      if (!term) return true;
      return [s.name, s.artist, s.location, s.music_type, s.description].filter(Boolean).join(" ").toLowerCase().includes(term);
    });
  ...
  const updateQueueItem = useCallback((itemId, nextVal) => {
    commitQueueItems((cur) => cur.map((item) => { if (item.id !== itemId) return item; const patch = typeof nextVal === "function" ? nextVal(item) : nextVal; return { ...item, ...patch }; }));
  }, [commitQueueItems]);

  const assignFiles = useCallback((kind, files) => {
    const arr = Array.from(files || []);
    if (!arr.length) return;
    if (kind === "songs") { const f = arr.filter((x) => ACCEPTED_AUDIO_EXTENSIONS.has(getExtension(x.name))); setSongFiles(f); setPendingAutoRun(true); appendLog("info", `Selected ${f.length} audio file(s).`); toast.success(`${f.length} song file(s) loaded.`); }
    else if (kind === "images") { const f = arr.filter((x) => ACCEPTED_IMAGE_EXTENSIONS.has(getExtension(x.name))); setImageFiles(f); setPendingAutoRun(true); appendLog("info", `Selected ${f.length} image file(s).`); toast.success(`${f.length} cover image(s) loaded.`); }
    else if (kind === "spreadsheet") { const f = arr.filter((x) => ACCEPTED_SPREADSHEET_EXTENSIONS.has(getExtension(x.name))); setSpreadsheetFiles(f.slice(0, 1)); setPendingAutoRun(true); appendLog("info", f[0] ? `Selected spreadsheet: ${f[0].name}.` : "No spreadsheet found."); if (f[0]) toast.success(`Spreadsheet "${f[0].name}" loaded.`); else toast.error("No spreadsheet found."); }
  }, [appendLog]);

  const songsDropzone = useDropzone({ noClick: true, noKeyboard: true, multiple: true, accept: { "audio/mpeg": [".mp3"] }, onDropAccepted: (f) => assignFiles("songs", f), onDropRejected: () => toast.error("Only MP3 files are supported.") });
  const imagesDropzone = useDropzone({ noClick: true, noKeyboard: true, multiple: true, accept: { "image/*": [".jpg", ".jpeg", ".png", ".webp"] }, onDropAccepted: (f) => assignFiles("images", f), onDropRejected: () => toast.error("Only JPG, PNG, or WEBP images.") });
  const spreadsheetDropzone = useDropzone({ noClick: true, noKeyboard: true, multiple: true, accept: { "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": [".xlsx"], "application/vnd.ms-excel": [".xls"], "text/csv": [".csv"] }, onDropAccepted: (f) => assignFiles("spreadsheet", f), onDropRejected: () => toast.error("Only XLSX, XLS, or CSV.") });
  ...
  const removeUploadedAssets = useCallback(async (song) => {
    const tasks = [];
    const a = extractStoragePath(song.mp3_url, AUDIO_BUCKET); if (a) tasks.push(supabase.storage.from(AUDIO_BUCKET).remove([a]));
    const c = extractStoragePath(song.cover_url, IMAGE_BUCKET); if (c) tasks.push(supabase.storage.from(IMAGE_BUCKET).remove([c]));
  ...
  const processSingleQueueItem = useCallback(async (itemId) => {
    ...
    try {
      ...
      const payload = { name: item.metadata.title, artist: item.metadata.artist, music_type: item.metadata.music_type, description: item.metadata.description || null, duration: item.metadata.duration || null, date: item.metadata.date, location: item.metadata.location || null, mp3_url: mp3Url, cover_url: coverUrl };
      const { error } = await supabase.from(SONGS_TABLE).insert(payload);
      if (error) {
        if (/duplicate|unique/i.test(error.message || "")) { updateQueueItem(itemId, { status: "skipped", progress: 100, error: "Duplicate row skipped", mp3_url: mp3Url, cover_url: coverUrl }); appendLog("warn", `Skipped duplicate: "${item.metadata.title}".`); return; }
        await Promise.allSettled([supabase.storage.from(AUDIO_BUCKET).remove([audioPath]), supabase.storage.from(IMAGE_BUCKET).remove([imagePath])]);
        throw error;
      }
      ...
      updateQueueItem(itemId, { status: "failed", progress: 100, error: error.message || "Upload failed." });
      appendLog("error", `Failed "${item.metadata.title}": ${error.message || "Unknown error"}.`);
    }
  }, [appendLog, updateQueueItem, uploadAsset]);

  const startQueueProcessing = useCallback(async (specificIds = null) => {
    const ids = specificIds || queueItemsRef.current.filter((i) => i.status === "ready" || i.status === "failed").map((i) => i.id);
    ...
    let cursor = 0;
    const worker = async () => { while (cursor < ids.length) { const idx = cursor++; const id = ids[idx]; if (!id) return; await processSingleQueueItem(id); } };
    try {
      await Promise.all(Array.from({ length: Math.min(UPLOAD_CONCURRENCY, ids.length) }, () => worker()));
      await fetchUploadedSongs({ silent: true });
      const fs = queueItemsRef.current.reduce((s, i) => { s[i.status] += 1; return s; }, { ready: 0, uploading: 0, success: 0, failed: 0, skipped: 0 });
  ...
  const prepareQueue = useCallback(async (startImmediately = false) => {
    if (!songFiles.length || !imageFiles.length || !spreadsheetFiles.length) { toast.error("Please select all three folders first."); return; }
    setIsPreparing(true);
    ...
    try {
      ...
      const rows = XLSX.utils.sheet_to_json(ws, { defval: "" }).map(normalizeSpreadsheetRow).filter((r) => r.filename);
      ...
      const audioMap = buildFileMap(songFiles), imageMap = buildFileMap(imageFiles);
      const stems = new Set(rows.map((r) => r.stem));
      const nextQueue = rows.map((row) => {
        const audioFile = audioMap.get(row.stem), imageFile = imageMap.get(row.stem);
        const sig = createSignature(row);
        const dup = duplicates.filenameStems.has(row.stem) || duplicates.signatures.has(sig);
        const errs = [];
        if (!row.title) errs.push("Missing title"); if (!row.artist) errs.push("Missing artist");
        if (!row.music_type) errs.push("Missing type"); if (!audioFile) errs.push("Missing MP3"); if (!imageFile) errs.push("Missing image");
        const stem = row.stem || slugify(`${row.title}-${row.artist}`) || `row-${row.rowIndex}`;
        const base = { id: `${stem}-${row.rowIndex}`, storageStem: stem, attempts: 0, metadata: row, audioFile, imageFile };
      ...
      songFiles.forEach((f) => { if (!stems.has(normalizeStem(f.name))) appendLog("warn", `Audio "${f.name}" has no spreadsheet row.`); });
      imageFiles.forEach((f) => { if (!stems.has(normalizeStem(f.name))) appendLog("warn", `Image "${f.name}" has no spreadsheet row.`); });
      ...
      if (startImmediately) { const readyIds = nextQueue.filter((i) => i.status === "ready").map((i) => i.id); if (readyIds.length) await startQueueProcessing(readyIds); else toast.error("No valid rows ready to upload."); }
    } catch (e) { toast.error(e.message || "Unable to prepare queue."); appendLog("error", e.message || "Queue preparation failed."); }
    finally { setIsPreparing(false); }
  }, [appendLog, commitQueueItems, fetchUploadedSongs, imageFiles, songFiles, spreadsheetFiles, startQueueProcessing]);

  useEffect(() => {
    if (!autoRun || !pendingAutoRun || isPreparing || isUploading) return;
    if (!songFiles.length || !imageFiles.length || !spreadsheetFiles.length) return;
    setPendingAutoRun(false); prepareQueue(true);
  }, [autoRun, imageFiles.length, isPreparing, isUploading, pendingAutoRun, prepareQueue, songFiles.length, spreadsheetFiles.length]);

  const retryFailedItems = useCallback(async () => {
    const ids = queueItemsRef.current.filter((i) => i.status === "failed" && i.audioFile && i.imageFile).map((i) => i.id);
    if (!ids.length) { toast.error("No retryable failed items."); return; }
    commitQueueItems((cur) => cur.map((i) => ids.includes(i.id) ? { ...i, status: "ready", progress: 0, error: "" } : i));
  ...
  const handleDeleteSong = useCallback(async (song) => {
    ...
    try {
      ...
      appendLog("warn", `Deleted "${song.name}".`); toast.success(`Deleted "${song.name}".`);
    } catch (e) { toast.error(e.message || "Unable to delete song."); }
  ...
  const handleSaveSongEdit = useCallback(async (draft) => {
    ...
    try {
      setSavingSongId(editingSong.id);
      const payload = { name: draft.name.trim(), artist: draft.artist.trim(), music_type: normalizeMusicType(draft.music_type), description: draft.description.trim() || null, duration: Number(draft.duration) || null, date: draft.date || null, location: draft.location.trim() || null };
      const { data, error } = await supabase.from(SONGS_TABLE).update(payload).eq("id", editingSong.id).select("id,name,artist,music_type,description,duration,date,location,mp3_url,cover_url,created_at").single();
      if (error) throw error;
      setUploadedSongs((cur) => cur.map((s) => s.id === editingSong.id ? data : s));
      setEditingSong(null); appendLog("success", `Updated "${payload.name}".`); toast.success("Song metadata updated.");
    } catch (e) { toast.error(e.message || "Unable to save changes."); }
    finally { setSavingSongId(null); }
  }, [appendLog, editingSong]);

  const clearSelections = () => { setSongFiles([]); setImageFiles([]); setSpreadsheetFiles([]); commitQueueItems([]); appendLog("info", "Cleared selections and queue."); };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "var(--app-shell-bg)", color: "var(--app-text-main)", fontFamily: "'DM Sans','Segoe UI',system-ui,sans-serif" }}>
      ...
      <input ref={songsInputRef} type="file" accept=".mp3" multiple hidden onChange={(e) => { assignFiles("songs", e.target.files); e.target.value = ""; }} />
      <input ref={imagesInputRef} type="file" accept=".jpg,.jpeg,.png,.webp" multiple hidden onChange={(e) => { assignFiles("images", e.target.files); e.target.value = ""; }} />
      <input ref={spreadsheetInputRef} type="file" accept=".xlsx,.xls,.csv" multiple hidden onChange={(e) => { assignFiles("spreadsheet", e.target.files); e.target.value = ""; }} />
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          <div className="upload-topbar">
            ...
            <div style={{ flex: 1 }} />
            <button type="button" onClick={() => prepareQueue(true)} disabled={isPreparing || isUploading}
              style={{ padding: "8px 14px", borderRadius: 999, border: "none", background: "linear-gradient(135deg,var(--app-accent-strong),var(--app-accent))", color: "#041307", cursor: isPreparing || isUploading ? "not-allowed" : "pointer", fontWeight: 800, fontSize: 12, boxShadow: "0 4px 14px rgba(var(--app-accent-rgb),0.3)", flexShrink: 0 }}>
              {isPreparing ? "Preparing…" : isUploading ? "Uploading…" : "Run Automation"}
            </button>
          </div>
            ...
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
              ...
              <div className="upload-hero-toolbar" style={{ position: "relative" }}>
                <div>
                  ...
                  <p style={{ margin: 0, color: "var(--app-text-muted)", lineHeight: 1.7, fontSize: 13, maxWidth: 600 }}>Select songs, images, and spreadsheet folders. The system matches files, uploads to Supabase Storage, writes metadata to SQL, and skips duplicates automatically.</p>
                </div>
                <div className="upload-automation-card">
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                    ...
                    <button type="button" onClick={() => setAutoRun((v) => !v)} style={{ width: 52, height: 30, borderRadius: 999, border: "none", background: autoRun ? "var(--app-accent)" : "rgba(148,163,184,0.28)", padding: 3, display: "flex", justifyContent: autoRun ? "flex-end" : "flex-start", cursor: "pointer", flexShrink: 0 }}>
                      <span style={{ width: 24, height: 24, borderRadius: "50%", background: autoRun ? "#041307" : "#fff", boxShadow: "0 4px 10px rgba(0,0,0,0.24)" }} />
                    </button>
                  </div>
                  <div style={{ marginTop: 8, color: "var(--app-text-muted)", fontSize: 11, lineHeight: 1.6 }}>{autoRun ? "Uploads automatically once all folders are selected." : "Inspect the queue before the batch starts."}</div>
                </div>
              </div>
            </motion.div>
              ...
              <div style={{ display: "grid", gap: 16 }}>
                ...
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.06 }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
                    <div>
                      <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>Source folders</h2>
                      <p style={{ margin: "4px 0 0", color: "var(--app-text-muted)", fontSize: 12 }}>Choose the three source folders to power automation.</p>
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <button type="button" onClick={clearSelections} disabled={isPreparing || isUploading} style={{ padding: "9px 12px", borderRadius: 999, border: "1px solid var(--app-border)", background: "transparent", color: "var(--app-text-muted)", cursor: isPreparing || isUploading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 12 }}>Clear</button>
                      <button type="button" onClick={() => prepareQueue(true)} disabled={isPreparing || isUploading} style={{ padding: "9px 14px", borderRadius: 999, border: "none", background: "linear-gradient(135deg,var(--app-accent-strong),var(--app-accent))", color: "#041307", cursor: isPreparing || isUploading ? "not-allowed" : "pointer", fontWeight: 800, fontSize: 12, boxShadow: "0 6px 18px rgba(var(--app-accent-rgb),0.24)" }}>
                        {isPreparing ? "Preparing…" : isUploading ? "Uploading…" : "Run automation"}
                      </button>
                    </div>
                  </div>
                  <div style={{ display: "grid", gap: 12 }}>
                    <FolderDropArea title="Songs folder" subtitle="song1.mp3, song2.mp3 …" count={songFiles.length} totalSize={totalSongSize} icon={Music4} buttonLabel="Pick songs" onPick={() => songsInputRef.current?.click()} disabled={isUploading} isDragActive={songsDropzone.isDragActive} isDragReject={songsDropzone.isDragReject} rootProps={songsDropzone.getRootProps()} inputProps={songsDropzone.getInputProps()} />
                    <FolderDropArea title="Images folder" subtitle="song1.jpg, song2.jpg …" count={imageFiles.length} totalSize={totalImageSize} icon={ImageIcon} buttonLabel="Pick images" onPick={() => imagesInputRef.current?.click()} disabled={isUploading} isDragActive={imagesDropzone.isDragActive} isDragReject={imagesDropzone.isDragReject} rootProps={imagesDropzone.getRootProps()} inputProps={imagesDropzone.getInputProps()} />
                    <FolderDropArea title="Excel details folder" subtitle="Columns: filename, title, artist, type, duration, date, description, location" count={spreadsheetFiles.length} totalSize={totalSpreadsheetSize} icon={FileSpreadsheet} buttonLabel="Pick spreadsheet" onPick={() => spreadsheetInputRef.current?.click()} disabled={isUploading} isDragActive={spreadsheetDropzone.isDragActive} isDragReject={spreadsheetDropzone.isDragReject} rootProps={spreadsheetDropzone.getRootProps()} inputProps={spreadsheetDropzone.getInputProps()} />
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <button type="button" onClick={() => prepareQueue(false)} disabled={isPreparing || isUploading} style={{ padding: "8px 12px", borderRadius: 999, border: "1px solid var(--app-border)", background: "transparent", color: "var(--app-text-main)", cursor: isPreparing || isUploading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 12 }}>Rebuild</button>
                      <button type="button" onClick={retryFailedItems} disabled={isPreparing || isUploading} style={{ padding: "8px 12px", borderRadius: 999, border: "1px solid rgba(248,113,113,0.26)", background: "rgba(248,113,113,0.08)", color: "#f87171", cursor: isPreparing || isUploading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 12 }}>Retry failed</button>
                    </div>
                  </div>
                  <div className="upload-scrollable" style={{ maxHeight: 480, overflow: "auto", display: "grid", gap: 10 }}>
                    {!queueItems.length ? (
                      <div style={{ borderRadius: 12, border: "1px dashed var(--app-border)", padding: "24px 16px", textAlign: "center", color: "var(--app-text-muted)", fontSize: 13 }}>Select the three folders to generate the automation queue.</div>
                    ) : (
                      <>
                            ...
                            <div key={item.id} className="upload-hover-card" style={{ borderRadius: 12, border: "1px solid var(--app-border)", background: "rgba(255,255,255,0.02)", padding: 14 }}>
                              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                                <div style={{ minWidth: 0 }}>
                                  ...
                                  <div style={{ marginTop: 5, color: "var(--app-text-muted)", fontSize: 11, lineHeight: 1.6 }}><strong>{item.metadata.filename}</strong> · {item.metadata.artist || "Missing"} · {item.metadata.music_type || "Missing"} · {formatDuration(item.metadata.duration)}</div>
                                  {item.error && <div style={{ marginTop: 6, color: item.status === "skipped" ? "#fbbf24" : "#f87171", fontSize: 11 }}>{item.error}</div>}
                                </div>
                              ...
                              <div style={{ height: 6, borderRadius: 999, background: "rgba(148,163,184,0.14)", overflow: "hidden", marginTop: 10 }}>
                                <div style={{ width: `${Math.min(item.progress, 100)}%`, height: "100%", borderRadius: 999, background: item.status === "failed" ? "linear-gradient(90deg,#ef4444,#f87171)" : item.status === "skipped" ? "linear-gradient(90deg,#d97706,#fbbf24)" : "linear-gradient(90deg,var(--app-accent-strong),var(--app-accent))", transition: "width 0.25s" }} />
                              </div>
                              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10, alignItems: "center" }}>
                                ...
                                {item.status === "failed" && item.audioFile && item.imageFile && (
                                  <button type="button" onClick={async () => { updateQueueItem(item.id, { status: "ready", progress: 0, error: "" }); await startQueueProcessing([item.id]); }} disabled={isUploading}
                                    style={{ marginLeft: "auto", padding: "6px 10px", borderRadius: 999, border: "1px solid rgba(var(--app-accent-rgb),0.24)", background: "rgba(var(--app-accent-rgb),0.1)", color: "var(--app-accent)", cursor: isUploading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 11 }}>Retry</button>
                                )}
                              </div>
                            </div>
                          );
                        ...
                        {queueItems.length > MAX_QUEUE_PREVIEW && (
                          <div style={{ borderRadius: 12, padding: "10px 12px", background: "rgba(var(--app-accent-rgb),0.08)", color: "var(--app-accent)", fontSize: 11, fontWeight: 700, textAlign: "center" }}>Showing first {MAX_QUEUE_PREVIEW} of {queueItems.length} items.</div>
                        )}
                      </>
                    )}
                  </div>
              ...
              <aside className="upload-right-col" style={{ display: "grid", gap: 16, alignSelf: "start", position: "sticky", top: 80 }}>
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.14 }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                    <div style={{ width: 38, height: 38, borderRadius: 12, display: "grid", placeItems: "center", background: "rgba(var(--app-accent-rgb),0.12)", color: "var(--app-accent)", flexShrink: 0 }}><Database size={16} /></div>
                    <div>
                  ...
                  <div className="upload-scrollable" style={{ maxHeight: 340, overflow: "auto", display: "grid", gap: 8 }}>
                    {!logs.length ? (
                      <div style={{ borderRadius: 12, border: "1px dashed var(--app-border)", padding: "16px 14px", color: "var(--app-text-muted)", fontSize: 12 }}>Logs will appear as soon as you prepare the queue.</div>
                    ) : logs.map((log) => (
                      <div key={log.id} style={{ borderRadius: 12, border: "1px solid var(--app-border)", background: "rgba(255,255,255,0.02)", padding: "10px 12px" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                          <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.04em", color: log.level === "error" ? "#f87171" : log.level === "warn" ? "#fbbf24" : log.level === "success" ? "var(--app-accent)" : "var(--app-text-muted)" }}>{log.level}</span>
                ...
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.18 }}
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    {["Matches MP3 and cover files by the filename column in Excel.", "Parses XLSX, XLS, and CSV automatically with batch-ready validation.", "Uploads assets to Supabase Storage and inserts rows into the songs SQL table.", "Skips duplicates using existing filenames and title/artist/type signatures.", "Supports retrying failed items without rebuilding the whole folder set."].map((note) => (
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                  <div className="upload-search-wrap">
                    ...
                    <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search uploaded songs…"
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {FILTER_TYPES.map((f) => (
                      <button key={f} type="button" className="upload-filter-pill" onClick={() => setTypeFilter(f)}
              ...
              <div style={{ display: "grid", gap: 12 }}>
                {songsLoading ? (
                  <div style={{ borderRadius: 12, padding: "14px", background: "rgba(var(--app-accent-rgb),0.08)", border: "1px solid rgba(var(--app-accent-rgb),0.18)", color: "var(--app-accent)", display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                    <Loader2 size={15} className="animate-spin" /> Loading songs from Supabase…
                  </div>
                ) : !filteredSongs.length ? (
                  <div style={{ borderRadius: 12, border: "1px dashed var(--app-border)", padding: "20px", color: "var(--app-text-muted)", textAlign: "center", fontSize: 13 }}>No songs match your search or filter.</div>
                  ...
                  <div key={song.id} className="upload-hover-card" style={{ borderRadius: 12, border: "1px solid var(--app-border)", background: "rgba(255,255,255,0.02)", padding: 14 }}>
                    <div className="upload-library-item">
                      <div style={{ width: 56, height: 56, borderRadius: 12, overflow: "hidden", background: "rgba(var(--app-accent-rgb),0.08)", display: "grid", placeItems: "center", flexShrink: 0 }}>
                        {song.cover_url ? <img src={song.cover_url} alt={song.name} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : <ImageIcon size={18} color="var(--app-accent)" />}
                      </div>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <div style={{ color: "var(--app-text-muted)", fontSize: 12, marginTop: 4, lineHeight: 1.6 }}>{song.artist} · {song.location || "Unknown"} · {formatDuration(song.duration)} · {song.date || "No date"}</div>
                        {song.description && <div style={{ color: "var(--app-text-muted)", fontSize: 11, marginTop: 4, lineHeight: 1.5 }}>{song.description}</div>}
                      </div>
                      <div className="upload-library-actions" style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
                        <button type="button" onClick={() => window.open(song.mp3_url, "_blank", "noopener,noreferrer")}
                        ...
                        <button type="button" onClick={() => setEditingSong(song)}
                        ...
                        <button type="button" onClick={() => handleDeleteSong(song)} disabled={deletingSongId === song.id}
                          ...
                          {deletingSongId === song.id ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />} Delete
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      <EditSongModal song={editingSong} onClose={() => setEditingSong(null)} onSave={handleSaveSongEdit} saving={savingSongId === editingSong?.id} />
    </div>
  );
}
